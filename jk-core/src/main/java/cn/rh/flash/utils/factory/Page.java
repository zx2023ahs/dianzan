package cn.rh.flash.utils.factory;
...
public class Page<T> {
    ...
    * @param current 当前页
    ...
    public void addFilter(String fieldName, SearchFilter.Operator operator, Object value, SearchFilter.Join join) {
        if (!StringUtil.isNullOrEmpty(value)) {
            addFilter(SearchFilter.build(fieldName, operator, value, join ) );
        }
    }
    ...
    public void addFilter(String fieldName, Object val , SearchFilter.Join join) {
        addFilter(fieldName, SearchFilter.Operator.EQ, val, join );
    }
